...
public class InstrumentationModelUtil {
     ...
     *            the model
     * @param type
     *            the class corresponding to the type of the objects
     * @return a list of all objects within the given model which conform to the given type
     */
    ...
    public static void recursiveBuildImm(ResourceDemandingBehaviour service, ServiceInstrumentationPoint sip) {
        for (AbstractAction action : service.getSteps_Behaviour()) {
            if (action instanceof LoopAction) {
                ...
                sip.getActionInstrumentationPoints()
                    .add(inner);
                ...
                ((BranchAction) action).getBranches_Branch()
                    .stream()
                    .forEach(branch -> {
